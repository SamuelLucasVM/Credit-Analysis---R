...
#2. Descreva a base de dados indicando o número de observações e as variáveis com os
#names, seus signicados e as correspondentes unidades de medida;
```

(TODO) Visualizando rapidamente a estrutura da base de dados, observamos as seguintes variáveis:
...
i.  não apresenta correlação linear significante com a variável `Age` (r= 0.002, p \> 1);

ii. não apresenta correlação linear significante com a variável `Education` (r= -0.008, p \< 1);

iii. apresenta correlação linear baixa com a variável `Cards` (r= 0.086, p \< 0.05);

iv. apresenta correlação linear significante com a variável `Income` (r= 0.464, p \< 0);

v. apresenta correlação linear significante com a variável `Limit` (r= 0.862, p \< 0);

vi. apresenta correlação linear significante com a variável `Rating` (r= 0.864, p \< 0);
...
#modelo2 <- update(modelo1, ~ . -Income)

#vif(modelo2)

#modelo3 <- update(modelo1, ~ . -Limit)

#vif(modelo3)

modelo4 <- update(modelo1, ~ . -Rating)
...
Na presente análise, seguiremos ajustando dois modelos excluindo do modelo inicial e completo 1, cada uma das variáveis explicativas em separado (`Petal.Length` e `Petal.Width`), mantendo a outra no modelo inicial. Vejamos:

\

## Modelo sem `Petal.Width` (modelo2) {#sec-modelo2selecionado}
...
> Observa-se que; para este modelo ajustado; todas as variáveis independentes são estatísticamente significativas com a adequação de ajuste do modelo aos dados igual a 83,8% (Adjusted R-squared: 0.838).

\

## Modelo sem `Petal.Length` (modelo3)
...
> Para este modelo e de maneira análoga ao modelo anterior; todas as variáveis independentes são estatísticamente significativas. Por outro lado, a adequação de ajuste do modelo aos dados igual a 70,33% (Adjusted R-squared: 0.7033) é inferior ao modelo anterior.

Podemos rever a análise de correlação e observar que a variável `Petal.Width` apresenta correlação linear um pouco menor com a variável resposta `Sepal.Length` quando comparada com a variável `Petal.Length`.

Seguiremos, para efeito didático com a exposição de formas de seleção de modelos.
...
Uma análise de variância (ANOVA) com o intuito de **selecionar um modelo (variáveis)** é possível ser utilizada quando se deseja comparar **modelos aninhados**; ou seja; quando um ou mais modelo é obtido como sub-conjunto do(s) outro(s). Vejamos como se aplica na presente análise.
...
> Observa-se pelo teste F que **deve-se rejeitar a hipótese nula** de que o modelo completo (com a variável `Petal.Width` inclusa) explica **tão bem quanto o sub-modelo sem a variável** `Petal.Width` (p \< 0,001). Em outras palavras, esta variável é estatísticamente significante para a regressão (ainda que haja problema de multicolinearidade quando na presença da variável `Petal.Length`).
...
> Ao realizar a ANOVA para verificar a significância da variável `Petal.Length` para regressão, observa-se resultado análogo ao da variável `Petal.Width`; ou seja; a variável `Petal.Length` é estatísticamente significante para a regressão (ainda que haja problema de multicolinearidade quando na presença da variável `Petal.Width`).
...
Agora, suponha que temos por objetivo prever os valores para os comprimentos das sépalas considerando os seguintes valores para as variáveis explicativas:

```{r}
#novas.preditoras <- data.frame(Sepal.Width=c(2.5, 3.0, 3.5), Petal.Length=c(2.0, 4.0, 3.0))
